...
int s_n[1000];
int n_s[1000];
int s_index=0,n_index=0,s_num=0,n_num=0;;
//railway问题管程构造函数
dp::dp(int r, int m) {
...
void dp::arrive(int i) {
    ...
    if (!i) {
        sleep(rate);//列车依次到达 
        cout << getpid() <<": "<<"train "<<s_n[s_index]<<" arrive"<<"\n";
        s_index++;
        if(s_index==s_num){//已发列车均到达，唤醒另一车站
          sleep(rate);
          *state[i] = waiting;
          self[i ^ 1]->Signal(i ^ 1);
          cout <<"south->north stop, north->south begin\n";
        }
    }
    else {
        sleep(rate); 
        cout << getpid() <<": "<<"train "<<n_s[n_index]<<" arrive"<<"\n";
        n_index++;
        if(n_index==n_num){
       	  sleep(rate);
          *state[i] = waiting;
          self[i ^ 1]->Signal(i ^ 1);
          cout <<"north->south stop, south->north begin\n";
        }
    }
